...
mod tests {
    ...
    fn test_to_multimodal_messages_single_image() {
        ...
        let content = result[0]["content"].as_array().expect("Content should be an array");
    ...
    fn test_to_multimodal_messages_multiple_images() {
        ...
        assert_eq!(content[1]["image_url"]["url"], "data:image/jpeg;base64,img1");
    ...
    fn test_to_multimodal_messages_images_only() {
        ...
        let content = result[0]["content"].as_array().expect("Content should be an array even without text");
        assert_eq!(content.len(), 1);
        assert_eq!(content[0]["type"], "image_url");
        assert_eq!(content[0]["image_url"]["url"], "data:image/webp;base64,img_only");
    }
    ...
    fn test_to_multimodal_messages_tool_calls() {
        ...
        let tool_calls = result[0]["tool_calls"].as_array().expect("Should have tool_calls");
    ...
    fn test_has_images() {
        ...
        let messages_with_empty_images = vec![
            ChatMessage {
                role: "user".to_string(),
                content: Some("Look at this".to_string()),
                reasoning: None,
