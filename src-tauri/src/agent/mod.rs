...
impl Agent {
    ...
    /// A non-streaming chat completion method for internal use (e.g., analysis, intent classification)
    pub async fn call_chat_completion(
        ...
        let selected_model = config
            .selected_model
            .clone()
            .unwrap_or_else(|| "gemini-2.0-flash-exp:free".to_string());

        // Handle Gemini models directly
        if selected_model.contains("gemini") && !selected_model.contains("openrouter") {
            let api_key = config
                .gemini_api_key
                .as_ref()
            ...
            let response = self
                ...
                .send()
                .await
                .map_err(|e| format!("Gemini API network error: {}", e))?;
        ...
        // Handle OpenRouter/Groq/Cerebras via OpenAI-compatible API
        let is_cerebras = selected_model.contains("(Cerebras)");
        let is_groq = selected_model.contains("(Groq)");

        let (api_key, base_url, model) = if is_cerebras {
            let key = config
                .cerebras_api_key
                .as_ref()
                .ok_or("No Cerebras API key configured")?;
